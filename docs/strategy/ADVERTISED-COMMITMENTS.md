...
## 8.1 Whole-site List-1 audit — 2026-07-08

Ran the same List-1 truth pass the `/demos/` page got (#169) across the rest of the marketing
...
2. **"Hands off when unsure" — FIXED.** List-1 behavior is **explicit-intent handoff + an _offered_
   ...
   guess." **Test coverage (§8 #6) — now closed:** a golden-set eval was built into the bot's launch
   gate (`cushlabs-messenger-bot/scripts/qa-gate.ts`, PR #116, 2026-07-08) driving the real
   `/qa/simulate` pipeline. It gates the **load-bearing, reliable** half — "doesn't guess": out-of-scope
   asks (French/German) and unknowable facts (calendar availability) must state the boundary and never
   fabricate; plus no over-escalation on mere frustration and no `[QR:human]` chip-spam on answerable
   questions (full gate 41/41). **Empirical finding to keep in mind:** the soft "offers to connect when
   unsure" behavior is **non-deterministic** — the model engages/diagnoses first and offers the chip only
   situationally (an EN frustrated message did; a near-identical es-MX one did not). The **explicit**
   human-request path is deterministic and separately tested (`handoff-triggers.test.ts`). So the FAQ's
